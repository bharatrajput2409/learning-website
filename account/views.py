...
def sendemailtoallusers(request):
    if request.user.is_authenticated and request.user.is_superuser==True:
        username=request.user.username
        content=request.POST['content']
        print(content)
        subject='System mail from bharatrajput2409.pythonanywhere.com'
        # reciverlist=[]
        # allusers=User.objects.only("username","email").all()
        # for k in allusers:
        #     reciverlist=[k.email]
        #     contentforuser='Hi, <span style="color:red;">'+k.username+'</span>'+"!"+'<br>'+content+'<br>'+'<a href="bharatrajput2409.pythonanywhere.com">click to visit our website</a>'
        #     # email=EmailMessage(subject,contentforuser,'bharatsinghnitk@gmail.com',reciverlist)
        #     # email.content_subtype= "html"
        #     # email.send(fail_silently=True)
        send_mail(subject,content,'bharatsinghnitk@gamil.com',['bharatrajput2409@gmail.com'],fail_silently=False)
        return redirect("/account/"+ username +"/dashboard")
    else:
        return render(request,'account/notauthorised.html')
